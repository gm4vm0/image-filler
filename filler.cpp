...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
  ...
  RGBAPixel seedColor = config.seedpoint.color;
  ...
  while (!os.IsEmpty()) {
    ...
    // if (p.color.distanceTo(seedColor) > config.tolerance) continue;

    RGBAPixel* pixel = img->getPixel(x, y);
    *pixel = (*config.picker)(p);
    visited.at(x).at(y) = true;

    if (y > 0 && !visited.at(x).at(y - 1) && img->getPixel(x, y - 1)->distanceTo(seedColor) <= config.tolerance)
      os.Add(PixelPoint(x, y - 1, *img->getPixel(x, y - 1)));
    if (x < img->width() - 1 && !visited.at(x + 1).at(y) && img->getPixel(x + 1, y)->distanceTo(seedColor) <= config.tolerance)
      os.Add(PixelPoint(x + 1, y, *img->getPixel(x + 1, y)));
    if (y < img->height() - 1 && !visited.at(x).at(y + 1) && img->getPixel(x, y + 1)->distanceTo(seedColor) <= config.tolerance)
      os.Add(PixelPoint(x, y + 1, *img->getPixel(x, y + 1)));
    if (x > 0 && !visited.at(x - 1).at(y) && img->getPixel(x - 1, y)->distanceTo(seedColor) <= config.tolerance)
      os.Add(PixelPoint(x - 1, y, *img->getPixel(x - 1, y)));

    if (++framecount % config.frameFreq == 0) anim.addFrame(*img);
  }
